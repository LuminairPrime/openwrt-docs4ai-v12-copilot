...
| AI data | **Reads** all three AI fields from L2 frontmatter for AGENTS.md tool descriptions |
| Parallelisable | Yes |
| Key env vars | `OUTDIR` |
...
| AI data | **Reads** all three AI fields for routing index entries |
...
| Depends on | 06 |
| Depended on by | — (terminal output artefact) |
| AI data | **Reads** `ai_summary` (included in llms.txt snippets) |
...
| AI data | **Reads** all three AI fields — checks for presence in enriched L2 files |
