...
| REMOTE-008 | deferred | validation-warning | `docker_rpc.uc` from `luci-app-dockerman` still emits one soft warning under standalone `ucode` syntax validation | Investigated after remote run `22870498903`. The file appears intentionally dual-mode and is consumed by LuCI as rpc objects such as `docker` and `docker.container`, so the remaining warning currently looks like a validation-context mismatch rather than a broad documentation failure. Leave as a truthful non-blocking warning unless stronger evidence emerges. |
