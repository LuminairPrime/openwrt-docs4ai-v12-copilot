...
<!-- Source: qa skill - auto-generated by install-claude-skills.ps1 -->
<!-- Purpose: Test app, find bugs, generate regression tests -->
...
**If BOOTSTRAP_DECLINED** appears: Print "Test bootstrap previously declined â€” skipping." **Skip the rest of bootstrap.**

**If NO runtime detected** (no config files found): Use AskUserQuestion:
"I couldn't detect your project's language. What runtime are you using?"
Options: A) Node.js/TypeScript B) Ruby/Rails C) Python D) Go E) Rust F) PHP G) Elixir H) This project doesn't need tests.
If user picks H â†’ write `.gstack/no-test-bootstrap` and continue without tests.

**If runtime detected but no test framework â€” bootstrap:**
...
| Rust | cargo test (built-in) + mockall | â€” |
| PHP | phpunit + mockery | pest |
| Elixir | ExUnit (built-in) + ex_machina | â€” |

### B3. Framework selection

Use AskUserQuestion:
"I detected this is a [Runtime/Framework] project with no test framework. I researched current best practices. Here are the options:
A) [Primary] â€” [rationale]. Includes: [packages]. Supports: unit, integration, smoke, e2e
B) [Alternative] â€” [rationale]. Includes: [packages]
C) Skip â€” don't set up testing right now
RECOMMENDATION: Choose A because [reason based on project context]"

If user picks C â†’ write `.gstack/no-test-bootstrap`. Tell user: "If you change your mind later, delete `.gstack/no-test-bootstrap` and re-run." Continue without tests.

If multiple runtimes detected (monorepo) â†’ ask which runtime to set up first, with option to do both sequentially.
...
If package installation fails â†’ debug once. If still failing â†’ revert with `git checkout -- package.json package-lock.json` (or equivalent for the runtime). Warn user and continue without tests.
...
3. **For each file:** Write one test that tests real behavior with meaningful assertions. Never `expect(x).toBeDefined()` â€” test what the code DOES.
4. Run each test. Passes â†’ keep. Fails â†’ fix once. Still fails â†’ delete silently.
...
If tests fail â†’ debug once. If still failing â†’ revert all bootstrap changes and warn user.
...
If `.github/` exists (or no CI detected â€” default to GitHub Actions):
...
If non-GitHub CI detected â†’ skip CI generation with note: "Detected {provider} â€” CI pipeline generation supports GitHub Actions only. Add test step to your existing pipeline manually."

### B6. Create TESTING.md

First check: If TESTING.md already exists â†’ read it and update/append rather than overwriting. Never destroy existing content.

Write TESTING.md with:
- Philosophy: "100% test coverage is the key to great vibe coding. Tests let you move fast, trust your instincts, and ship with confidence â€” without them, vibe coding is just yolo coding. With tests, it's a superpower."
...
First check: If CLAUDE.md already has a `## Testing` section â†’ skip. Don't duplicate.

Append a `## Testing` section:
- Run command and test directory
- Reference to TESTING.md
- Test expectations:
  - 100% test coverage is the goal â€” tests make vibe coding safe
...
2. **Identify affected pages/routes** from the changed files:
   - Controller/route files â†’ which URL paths they serve
   - View/template/component files â†’ which pages render them
   - Model/service files â†’ which pages use those models (check controllers that reference them)
   - CSS/style files â†’ which pages include those stylesheets
   - API endpoints â†’ test them directly with `$B js "await fetch('/api/...')"`
   - Static pages (markdown, HTML) â†’ navigate to them directly

   **If no obvious pages/routes are identified from the diff:** Do not skip browser testing. The user invoked /qa because they want browser-based verification. Fall back to Quick mode â€” navigate to the homepage, follow the top 5 navigation targets, check console for errors, and test any interactive elements found. Backend, config, and infrastructure changes affect app behavior â€” always verify the app still works.

3. **Detect the running app** â€” check common local dev ports:
   ```bash
...
5. **Cross-reference with commit messages and PR description** to understand *intent* â€” what should the change do? Verify it actually does that.
...
- `__next` in HTML or `_next/data` requests â†’ Next.js
- `csrf-token` meta tag â†’ Rails
- `wp-content` in URLs â†’ WordPress
- Client-side routing with no page reloads â†’ SPA
...
1. **Visual scan** â€” Look at the annotated screenshot for layout issues
...
**Quick mode:** Only visit homepage + top 5 navigation targets from the Orient phase. Skip the per-page checklist â€” just check: loads? Console errors? Broken links visible?
...
3. **Write console health summary** â€” aggregate all console errors seen across pages
4. **Update severity counts** in the summary table
5. **Fill in report metadata** â€” date, duration, pages visited, screenshot count, framework
6. **Save baseline** â€” write `baseline.json` with:
   ```json
   {
     "date": "YYYY-MM-DD",
     "url": "<target>",
...
- Monitor `_next/data` requests in network â€” 404s indicate broken data fetching
- Test client-side navigation (click links, don't just `goto`) â€” catches routing issues
- Check for CLS (Cumulative Layout Shift) on pages with dynamic content

### Rails
- Check for N+1 query warnings in console (if development mode)
- Verify CSRF token presence in forms
- Test Turbo/Stimulus integration â€” do page transitions work smoothly?
...
- Use `snapshot -i` for navigation â€” `links` command misses client-side routes
- Check for stale state (navigate away and back â€” does data refresh?)
- Test browser back/forward â€” does the app handle history correctly?
...
9. **Never delete output files.** Screenshots and reports accumulate â€” that's intentional.
10. **Use `snapshot -C` for tricky UIs.** Finds clickable divs that the accessibility tree misses.
11. **Show screenshots to the user.** After every `$B screenshot`, `$B snapshot -a -o`, or `$B responsive` command, use the Read tool on the output file(s) so the user can see them inline. For `responsive` (3 files), Read all three. This is critical â€” without it, screenshots are invisible to the user.
12. **Never refuse to use the browser.** When the user invokes /qa or /qa-only, they are requesting browser-based testing. Never suggest evals, unit tests, or other alternatives as a substitute. Even if the diff appears to have no UI changes, backend changes affect app behavior â€” always open the browser and test.
...
```
.gstack/qa-reports/
â”œâ”€â”€ qa-report-{domain}-{YYYY-MM-DD}.md    # Structured report
â”œâ”€â”€ screenshots/
â”‚   â”œâ”€â”€ initial.png                        # Landing page annotated screenshot
â”‚   â”œâ”€â”€ issue-001-step-1.png               # Per-issue evidence
â”‚   â”œâ”€â”€ issue-001-result.png
â”‚   â”œâ”€â”€ issue-001-before.png               # Before fix (if fixed)
â”‚   â”œâ”€â”€ issue-001-after.png                # After fix (if fixed)
â”‚   â””â”€â”€ ...
